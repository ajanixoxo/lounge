...
export const MenuSection: React.FC<MenuSectionProps> = ({
 ...
 const [openCategoryTitle, setOpenCategoryTitle] = useState<
  string | null
 >(null);
 ...
 const filteredCategories =
  selectedGroup === 'All'
   ...
   : section.categories.filter(
      (cat) => getGroup(cat.title) === selectedGroup,
     );
 ...
 return (
  <section className="pt-[50px] pb-16 px-6 md:px-24 max-w-7xl mx-auto relative overflow-hidden">
   {/* Watermark */}
   ...
   <motion.div
    ...
    className="mb-8"
   >
    <h2 className="text-6xl  font-serif italic text-white/5 absolute -translate-y-[40%] pointer-events-none select-none tracking-wider">
     {section.backgroundTitle}
    </h2>
    <h3 className="text-3xl md:text-4xl font-serif relative z-10 pt-4">
     {section.title} <span className="text-rixos-gold">Menu</span>
    </h3>
    ...
    <AnimatePresence mode="popLayout">
     {filteredCategories.map((cat) => (
      <CollapsibleCategory
       ...
       onToggle={() =>
        setOpenCategoryTitle(
         openCategoryTitle === cat.title ? null : cat.title,
        )
       }
      />
     ))}
